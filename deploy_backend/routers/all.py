...
def get_nearest_stops(
    lat: float = Query(..., description="Latitude pengguna", example=-7.7797),
    lon: float = Query(..., description="Longitude pengguna", example=110.3752),
...
def get_stop(stop_id: str = Path(..., example="HT_194")):
...
def get_poi_detail(poi_id: int = Path(..., example=10)):
...
class RouteRequest(BaseModel):
    origin_stop_id: str = Field(..., example="HT_194")
    origin_walk_min: float = 0.0
    dest_poi_id: int = Field(..., example=10)
    depart_hhmm: str = "09:00"
...
def get_schedule(
    stop_id: str = Query(..., example="HT_194"),
...
def get_routes_geojson(route_id: Optional[str] = Query(None, example="1A")):
...
def get_route_detail(route_dir: str = Path(..., example="1A_0")):
...
def get_routes_between(
    from_stop: str = Query(..., example="HT_194"),
    to_stop: str = Query(..., example="HT_001"),
):
